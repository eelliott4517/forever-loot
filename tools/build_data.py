...
Inputs (see README):
  raw/zone_<id>.json, raw/scrape.json   Wowhead Forever + Classic dungeon zone pages (drops by NPC)
  raw/raid_zones.json                   Wowhead Forever + Classic raid zone pages
  raw/raid_npcs.json                    Wowhead raid boss and loot chest pages (drop chances)
  raw/wowtbc/<slug>.json                wowtbc.gg Forever dungeon loot tables (new drops, drop chances)
    ...
    for fn in sorted(os.listdir(RAW)):
        ...
        if m:
            ...
            pages[f"forever_zone_{m.group(1)}"] = dict(
                drops=[slim_drop(x) for x in d["drops"]], npcs=d["npcs"], objects=[],
        ...
        for k, v in load_json(name).items():
            if k not in pages:
                pages[k] = dict(drops=[slim_drop(x) for x in v["drops"]], npcs=v["npcs"],
                                objects=v.get("objects", []), level=v.get("level"))
    return pages
...
def load_tbc(slug):
    ...
    return ctx["loot"][0], {g["id"]: g for g in ctx["gearData"]}
...
class Build:
    def __init__(self):
        ...
        self.boss_pages = load_json("raid_npcs.json")
        self.report = []
    ...
    def collect(self, d, raid=False):
        ...
        # 5) Datamined new items without a confirmed boss
        extra = Loot()
        hints = {}
        ...
        for acc in (trash, extra):
            for i, s in acc.sources.items():
                self.sources[i] |= s
        return (d, bosses, trash, extra, hints)
...
def main():
    ...
    all_ids = set(sources)
    print(f"checking {len(all_ids)} items against Wowhead's Forever database...", file=sys.stderr)
    forever_info = item_info.fetch(all_ids, "forever")
    classic_ids = [i for i in all_ids if forever_info.get(i, {}).get("missing")]
    print(f"{len(classic_ids)} items aren't in Forever; fetching their Classic tooltips...", file=sys.stderr)
    ...
    for iid in all_ids:
        ...
        if any(SOD_MARK.search(l) for l in lines):
            if iid not in build.era_listed:
    ...
    def emit(var, plan, raid):
        L.append(f"{var} = {{")
        for d, bosses, trash, extra, hints in plan:
            L.append("\t{")
            ...
            for b in bosses:
                ...
                if b.get("npc"):
                    parts.append("npc = { " + ", ".join(str(i) for i in b["npc"]) + " }")
                if b.get("wing"):
                    parts.append(f"wing = {lua_str(b['wing'])}")
                if b.get("tag"):
                    parts.append(f"tag = {lua_str(b['tag'])}")
                parts.append("loot = { " + ", ".join(str(i) for i in loot) + " }")
            ...
            if el:
                ...
                L.append('\t\t\t{ name = "New in Forever, boss not confirmed", unconfirmed = true, '
                         'tag = "Datamined on Wowhead", loot = { ' + ", ".join(str(i) for i in el) + " }"
    ...
    L.append("-- [itemID] = { name, quality, type, flag, sources, icon, tooltip lines (from Wowhead) }")
    ...
    for label, plan in (("dungeons", dungeon_plan), ("raids", raid_plan)):
        empty = [(d["key"], b["name"]) for d, bs, _, _, _ in plan for b in bs if not sort_items(b["loot"].items)]
        n_items = len({i for d, bs, t, e, _ in plan for x in list(bs) + [t, e] for i in sort_items(
            (x["loot"] if isinstance(x, dict) else x).items)})
        print(f"{label}: {len(plan)}, {n_items} items, {len(empty)} bosses without loot", file=sys.stderr)
